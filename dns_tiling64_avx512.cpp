          ...
          for (int j = 0; j < SIZE/4; j+=64){
                for(int q=0; q<64; q++){
                  for(int p=0; p<64; p++){
                    ...
                    __m512 temp_line = _mm512_set1_ps(temp);
                    for(int r=0; r<64; r+=16){
                      _mm512_storeu_ps(&arg_thread->matC[arg_thread->c+(i*SIZE+j)+(p*SIZE+r)], _mm512_add_ps(
                          _mm512_loadu_ps(&arg_thread->matC[arg_thread->c+(i*SIZE+j)+(p*SIZE+r)]),
                          _mm512_mul_ps(
                              temp_line, 
                              _mm512_loadu_ps(&arg_thread->matB[arg_thread->b+(k*SIZE+j)+(q*SIZE+r)])
                              ) 
                          )
                      );
                    }
